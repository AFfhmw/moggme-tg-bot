...
async def analyze_face_async(image_bytes):
    """Spawns a separate Python process for face analysis. Process dies after, freeing all memory."""
    try:
        proc = await asyncio.create_subprocess_exec(
            sys.executable, "analyze_worker.py",
            stdin=asyncio.subprocess.PIPE,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        stdout, stderr = await asyncio.wait_for(proc.communicate(input=image_bytes), timeout=90)
        if proc.returncode != 0:
            err = stderr.decode(errors="replace").strip()[:200]
            return {"error": f"Ошибка анализа (code {proc.returncode}): {err}"}
        data = stdout.decode().strip()
        if not data:
            return {"error": "Worker не вернул результат"}
        import json
        result = json.loads(data)
        if "annotated_b64" in result:
            result["annotated_image_bytes"] = base64.b64decode(result.pop("annotated_b64"))
        return result
        ...
        try:
            ...
            pass
        return {"error": "Анализ занял слишком много времени. Попробуй другое фото."}
    except Exception as e:
